...
Given("the user is logged in and on the Contact List page", async function () {
  ...
  await this.page.fill("#email", 'hasibuninuba@mechanicspedia.com');
  await this.page.fill("#password", '1234567');
...
When("the user edits the contact information", async function () {
  ...
  await this.page.fill("#birthdate", editData.birthdate);
...
When(
  "the user creates a contact with {string}, {string}, {string}",
  async function (firstName, lastName, birthdate,) {
    // Click add contact button
    await this.page.click("#add-contact");
    ...
    this.currentContact = { firstName, lastName, birthdate };
  }
);
...
Then(
  "the contact birthdate {string} is displayed correctly",
  async function (expectedBirthdate) {
    const fullName = `${this.currentContact.firstName} ${this.currentContact.lastName}`;
    const rowSelector = `.contactTable tr:has-text("${fullName}")`;

    await this.page.waitForSelector(rowSelector, { timeout: 10000 });

    const row = this.page.locator(rowSelector);
    await expect(row).toContainText(expectedBirthdate);
  }
);
